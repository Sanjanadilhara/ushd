...
session_start();
	...
	if($_POST['type']=='1'){
		...
		while ($row = mysqli_fetch_array($result)) {
			...
			<a href='admin/upload/".$row['mname']."' target='_blank'><div><i class='fa-solid fa-file-lines fs-1'></i></div>
		...
		if (empty($_FILES['upfile']['name'])){
			...
			move_uploaded_file($tname, "admin/upload/".$pname);
      ...
      <div class='modal-body px-5'>
		...
		<div class=\"mb-3\">
			...
			<div><a href='admin/upload/".$row['mname']."'><i class=\"me-2 fa-solid fa-file-lines fs-1\"></i>".$row['mname']."</a></div>
		</div>
      </div>
