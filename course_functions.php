<?php
	...
	if($_POST['type']=='1'){
		...
		if(mysqli_num_rows(mysqli_query($conn, "SELECT * FROM c_course WHERE cid ='$id'"))>0){
			...
			if ($res) {
				...
				mysqli_query($conn, "INSERT INTO activitylog (user_name, name, activity) VALUE ((SELECT nic FROM user WHERE user_id='$uid'), (SELECT firstname FROM user WHERE user_id='$uid'), '$act')");
    ...
    if($res){
		...
		mysqli_query($conn, "INSERT INTO activitylog (user_name, name, activity) VALUE ((SELECT nic FROM user WHERE user_id='".$_SESSION['id']."'), (SELECT firstname FROM user WHERE user_id='".$_SESSION['id']."'), '$act')");
		echo "successfully deleted".$_SESSION['id'];
