<?php
	...
	if($_POST['type']=='1'){
		...
		if(mysqli_num_rows($result) == 1){
			...
			mysqli_query($conn, "INSERT INTO recover(uid, vcode) values(".$row['id'].", $code)");
	...
	if($_POST['type']=='2'){
		$result = mysqli_query($conn, "SELECT * FROM recover where vcode='".$_POST['code']."' AND uid=(select id from admin where email='".$_SESSION['email']."')");
		if(mysqli_num_rows($result) == 1){
			$row = mysqli_fetch_array($result);
